...
class MainAppScaffold extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    EdgeInsets padding = const EdgeInsets.symmetric(vertical: 24, horizontal: 8);
    return Scaffold(
      body: SafeArea(
        child: Column(
          children: [
...
class HomeView extends StatelessWidget {
  @override
  Widget build(BuildContext context) =>
      Container(alignment: Alignment.center, color: Colors.blue.shade300, child: Text("HOME"));
}

class NestedTabScaffold extends StatelessWidget {
  const NestedTabScaffold({Key? key, required this.child, required this.category, required this.subCategories})
      : super(key: key);
  final Widget child;
  ...
  Widget build(BuildContext context) => Column(
        children: [
          Row(children: subCategories.map((e) => _TabBtn('/$category/$e', fontSize: 14)).toList()),
...
class ProductsListPage extends StatelessWidget {
  ...
  late final List<_ProductBtn> _btns = List.generate(
    20,
    (index) => _ProductBtn("/$category/$subCategory/$productInfo/${Random().nextInt(999)}"),
  );
...
class _TabBtn extends StatelessWidget {
  const _TabBtn(this.path, {Key? key, this.label, this.padding, this.rootPath, this.exactMatch = false, this.fontSize})
      : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    bool isSelected = exactMatch ? urlNotifier.value == pathToMatch : urlNotifier.value.contains(pathToMatch);
  ...
  Widget build(BuildContext context) => Container(
      ...
      child: Column(
        children: [
          TextButton(
            child: Text('<< Back'),
            onPressed: () => urlNotifier.value = VRouter.of(context).previousUrl!,
          ),
          Spacer(),
